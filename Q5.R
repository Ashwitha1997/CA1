...
attach(heart_data)
plot(time, DEATH_EVENT, pch=19, col= "lightblue")


library("lattice")
attach(heart_data)
histogram(time,DEATH_EVENT, 
          data = heart_data, 
          main = "Distribution of heart failure data", 
          xlab = "Patients follow up time", 
...
normality_test <- shapiro.test(heart_data$time)
